...
class EntrepriseSiegeController extends Controller
{
    ...
    public function store(EntrepriseSiegeRequest $request)
    {
        ...
        // Créer le siège
        $siege = $this->repository->create($validated);

        // ✅ NOUVEAU : si c'est un Revendeur, attacher le siège à son portefeuille
        if ($user->isSeller()) {
            // Éviter les doublons (sécurité)
            $alreadyAttached = \DB::table('seller_sieges')
        ...
        ActivityLogService::log(
            action: 'create',
            modelType: 'EntrepriseSiege',
            modelId: $siege->ID,
            modelLabel: $siege->Nom,
        );
