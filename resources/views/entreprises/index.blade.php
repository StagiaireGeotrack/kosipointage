...
<x-app-layout>
                                            ...
                                            @if (auth()->user()->isTrueSuperAdmin() )
                                                @if ($entreprise->deleted)
                                                    <button type="button" class="btn btn-link text-danger p-0 border-0"
                                                            onclick="setResetAction('{{ route('entreprises.reset', $entreprise->ID) }}', '{{ $entreprise->Nom }}')"
                                                ...
                                                @else
                                                    <button type="button" class="btn btn-link text-danger p-0 border-0"
                                                            onclick="setDeleteAction('{{ route('entreprises.destroy', $entreprise->ID) }}', '{{ $entreprise->Nom }}')"
                                                            data-bs-toggle="modal"
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <p>{{ __('Voulez-vous vraiment supprimer ce site ou établissement ?') }}</p>
                                <p class="fw-bold" id="details_site"></p>
                            </div>
                ...
                <div class="modal fade" id="resetModal" tabindex="-1" aria-labelledby="resetModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <p>{{ __('Voulez-vous vraiment réinitialiser ce site ou établissement ?') }}</p>
                                <p class="fw-bold" id="site_"></p>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __('Annuler') }}</button>
    ...
    @push("scripts")
        ...
        function setDeleteAction(url, name) {
            ...
            document.getElementById('details_site').textContent = name;
        }

        function setResetAction(url, name) {
            document.getElementById('resetForm').action = url;
            document.getElementById('site_').textContent = name;
        }
