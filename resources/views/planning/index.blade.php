...
<style>
    ...
    /* Vue Mois - compact */
    .month-cell { min-width: 38px !important; max-width: 38px !important; padding: 2px !important; }
    .month-dot {
        display: flex; align-items: center; justify-content: center;
        width: 100%; height: 20px; border-radius: 4px;
        font-size: 9px; font-weight: 600; cursor: pointer; transition: all 0.15s ease;
    }
    .month-dot:hover { transform: scale(1.15); box-shadow: 0 2px 8px rgba(0,0,0,0.15); }
    .month-dot.work { background: #DCFCE7; color: #166534; }
    .month-dot.pause { background: #FEF3C7; color: #92400E; }
    .month-dot.conge { background: #FEF3C7; color: #92400E; }
    .month-dot.rtt { background: #FCE7F3; color: #9D174D; }
    .month-dot.maladie { background: #FEE2E2; color: #991B1B; }
    .month-dot.absence { background: #DBEAFE; color: #1E40AF; }
    .month-dot.formation { background: #EDE9FE; color: #5B21B6; }
    .month-dot.mission { background: #FFEDD5; color: #9A3412; }
    .month-dot.rest { background: #F1F5F9; color: #94A3B8; }

    /* Filtres légende */
    .legend-filter {
        cursor: pointer; user-select: none;
        transition: all 0.15s ease; opacity: 0.85;
    }
    .legend-filter:hover {
        transform: translateY(-1px);
        box-shadow: 0 2px 6px rgba(0,0,0,0.1); opacity: 1;
    }
                ...
                <table class="w-full border-collapse text-left text-xs" id="planning-table">
                    <thead>
                        <tr class="border-b border-slate-200 bg-slate-50 text-slate-600 font-semibold">
                            ...
                            @if(isset($days))
                                @foreach($days as $day)
                                <th class="p-3 text-center border-l border-slate-200 {{ $view === 'month' ? 'month-cell' : 'min-w-[130px]' }}">
                                    @if($view === 'month')
                                        <span class="block font-bold text-xs">{{ $day['day_num'] }}</span>
                                        <span class="block text-[9px] font-normal text-slate-400">{{ $day['short'] }}</span>
                                    @else
                                        {{ $day['short'] }}
                    ...
                    <tbody class="divide-y divide-slate-100" id="planning-body">
                        @if(isset($servicesData) && count($servicesData) > 0)
                            ...
                            @endphp
                            <tr class="hover:bg-slate-50/50 employee-row"
                                ...
                                @foreach($days as $day)
                                <td class="border-l border-slate-100 align-top {{ $view === 'month' ? 'month-cell' : 'p-1 min-w-[130px]' }}">
                                    @if(isset($emp['schedule'][$day['date']]))
                                        @if($view === 'month')
                                            @php
                                                $firstItem = $emp['schedule'][$day['date']][0] ?? null;
                                                $dotClass = 'rest'; $dotText = '·'; $tooltip = '';
                                                if ($firstItem) {
                                                    $type = $firstItem['type'] ?? 'rest';
                                                    $dotClass = match($type) {
                                                        'work' => 'work', 'pause' => 'pause', 'conge' => 'conge',
                                                        'rtt' => 'rtt', 'maladie' => 'maladie', 'absence' => 'absence',
                                                        'formation' => 'formation', 'mission', 'deplacement' => 'mission',
                                                        default => 'rest',
                                                    };
                                                    $dotText = match($type) {
                                                        'work' => 'T', 'pause' => 'P', 'conge' => 'C', 'rtt' => 'R',
                                                        'maladie' => 'M', 'absence' => 'A', 'formation' => 'F',
                                                        'mission', 'deplacement' => 'D', default => '·',
                                                    };
                                                    $tooltip = $firstItem['time'] ?? ($firstItem['title'] ?? '');
                                                }
                                            @endphp
                                            <div class="month-dot {{ $dotClass }}" title="{{ $day['date'] }} : {{ $tooltip }}">
                                                {{ $dotText }}
                                            </div>
                                        @else
                                            <div class="space-y-1">
                                                @foreach($emp['schedule'][$day['date']] as $item)
                                                    ...
                                                    @elseif($item['type'] === 'formation')
                                                        <div class="schedule-item has-event p-2 rounded-lg bg-purple-100 border border-purple-200 text-purple-900 text-center relative" data-id="{{ $item['id'] ?? '' }}" data-type="evenement">
                                                            <span class="event-action-btn text-red-500" onclick="event.stopPropagation(); annulerEvenement({{ $item['id'] }}, '{{ $item['title'] ?? 'Formation' }}')" title="Annuler cet événement">
                                                    ...
                                                    @elseif($item['type'] === 'deplacement' || $item['type'] === 'mission')
                                                        <div class="schedule-item has-event p-2 rounded-lg bg-orange-50 border border-orange-200 text-orange-900 text-center relative" data-id="{{ $item['id'] ?? '' }}" data-type="evenement">
                                                            <span class="event-action-btn text-red-500" onclick="event.stopPropagation(); annulerEvenement({{ $item['id'] }}, '{{ $item['title'] ?? 'Déplacement' }}')" title="Annuler cet événement">
                                                                <i data-lucide="x" class="w-3 h-3"></i>
                                                            </span>
                                                            <div class="font-bold text-[11px] flex items-center justify-center gap-1">
                                                    ...
                                                    @elseif($item['type'] === 'reunion')
                                                        <div class="schedule-item has-event p-2 rounded-lg bg-blue-100 border border-blue-200 text-blue-900 text-center relative" data-id="{{ $item['id'] ?? '' }}" data-type="evenement">
                                                            <span class="event-action-btn text-red-500" onclick="event.stopPropagation(); annulerEvenement({{ $item['id'] }}, '{{ $item['title'] ?? 'Réunion' }}')" title="Annuler cet événement">
                                                                <i data-lucide="x" class="w-3 h-3"></i>
                                                            </span>
                                                            <div class="font-bold text-[11px] flex items-center justify-center gap-1">
                                                                <i data-lucide="users" class="w-3.5 h-3.5 text-blue-600 flex-shrink-0"></i> {{ $item['title'] ?? 'Réunion' }}
                                                            </div>
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-8">
                            ...
                            <input type="text" name="titre" id="event_titre" class="form-control" required placeholder="Ex: Formation sécurité">
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="datetime-local" name="debut" id="event_debut" class="form-control" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Fin *</label>
                            <input type="datetime-local" name="fin" id="event_fin" class="form-control" required>
                        </div>
                        <div class="col-md-12">
                            <label class="form-label fw-semibold">Description</label>
                            <textarea name="description" id="event_description" class="form-control" rows="2" placeholder="Détails de l'événement..."></textarea>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Couleur</label>
                            <input type="color" name="couleur" id="event_couleur" class="form-control form-control-color" value="#8B5CF6">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Toute la journée</label>
                            <div class="form-check mt-2">
                                <input type="checkbox" name="toute_la_journee" id="event_all_day" class="form-check-input" value="1">
                        ...
                        <div class="col-md-12">
                            ...
                            <select name="employe_ids[]" id="event_employes" class="form-select" multiple size="6" required>
                                @if(isset($servicesData))
                                    @foreach($servicesData as $service)
                            ...
                            <small class="text-muted">Maintenez Ctrl (ou Cmd) pour sélectionner plusieurs employés</small>
                        </div>
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.addEventListener('click', function(e) {
            ...
            content.innerHTML = '<div class="text-center py-8"><div class="spinner-border text-primary" role="status"></div><p class="text-xs text-slate-400 mt-2">Chargement...</p></div>';

            fetch('/planning/event-detail/' + eventId + '/' + eventType)
                .then(r => r.json())
                .then(data => {
                    if (!data || !data.type) {
                        content.innerHTML = '<div class="text-center py-8 text-slate-400"><i data-lucide="alert-circle" class="w-10 h-10 mx-auto mb-2 text-slate-300"></i><p class="text-sm">Aucun détail disponible</p></div>';
                ...
                .catch(() => {
                    content.innerHTML = '<div class="text-center py-8 text-slate-400"><i data-lucide="alert-circle" class="w-10 h-10 mx-auto mb-2 text-red-300"></i><p class="text-sm text-red-600">Erreur de chargement</p></div>';
                    if (typeof lucide !== 'undefined') lucide.createIcons();
                });
        });
    });
