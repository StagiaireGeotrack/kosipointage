{{-- resources/views/supervisors/index.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="fw-semibold fs-4 text-dark mb-0">
                {{ __('Responsables de service') }}
            </h2>
            <a href="{{ route('supervisors.create') }}" class="btn btn-primary">
                {{ __('Nouveau responsable de service') }}
            </a>
        </div>
    </x-slot>

    <div class="p-2">
        <div class="card shadow-sm">
            <div class="card-body">

                <!-- Tableau des responsables -->
                <div class="table-responsive">
                    <table class="table table-hover">
                        <thead class="table-light">
                            <tr>
                                <th class="text-uppercase small fw-semibold text-secondary">
                                    {{ __('E-mail') }}
                                </th>
                                <th class="text-uppercase small fw-semibold text-secondary d-none d-md-table-cell">
                                    {{ __('Services affectés') }}
                                </th>
                                <th class="text-uppercase small fw-semibold text-secondary d-none d-lg-table-cell">
                                    {{ __('Date de création') }}
                                </th>
                                <th class="text-uppercase small fw-semibold text-secondary">
                                    {{ __('Statut') }}
                                </th>
                                <th class="text-uppercase small fw-semibold text-secondary">
                                    {{ __('Actions') }}
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($supervisors as $supervisor)
                                <tr>
                                    <td class="align-middle" style="max-width: 220px;">
                                        <span class="d-inline-block text-truncate w-100" title="{{ $supervisor->Identifiant_email }}">
                                            {{ $supervisor->Identifiant_email }}
                                        </span>
                                    </td>
                                    <td class="align-middle d-none d-md-table-cell">
                                        @if($supervisor->supervisorServices && $supervisor->supervisorServices->count() > 0)
                                            @foreach($supervisor->supervisorServices as $service)
                                                <span class="badge bg-secondary me-1 mb-1">{{ $service->name }}</span>
                                            @endforeach
                                        @else
                                            <span class="text-muted fst-italic">{{ __('Aucun service') }}</span>
                                        @endif
                                    </td>
                                    <td class="align-middle d-none d-lg-table-cell">
                                        @if($supervisor->created_at)
                                            <x-local-date-time :datetime="$supervisor->created_at"/>
                                        @endif
                                    </td>
                                    <td class="align-middle">
                                        @if ($supervisor->Actived && !$supervisor->deleted)
                                            <span class="badge bg-success">{{ __('Activé') }}</span>
                                        @else
                                            <span class="badge bg-danger">{{ __('Désactivé') }}</span>
                                        @endif
                                        @if ($supervisor->deleted)
                                            <span class="badge bg-danger">{{ __('Supprimé') }}</span>
                                        @endif
                                    </td>
                                    <td class="align-middle">
                                        <div class="d-flex gap-2">
                                            <a href="{{ route('supervisors.show', $supervisor->ID) }}" class="text-primary" title="Voir">
                                                <svg class="bi" width="20" height="20" fill="currentColor" viewBox="0 0 20 20">
                                                    <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                                                    <path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" />
                                                </svg>
                                            </a>

                                            <a href="{{ route('supervisors.edit', $supervisor->ID) }}" class="text-warning" title="Modifier">
                                                <svg class="bi" width="20" height="20" fill="currentColor" viewBox="0 0 20 20">
                                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                                </svg>
                                            </a>

                                            @if($supervisor->deleted)
                                                <button type="button" class="btn btn-link text-success p-0 border-0"
                                                    onclick="setResetAction('{{ route('supervisors.reset', $supervisor->ID) }}', '{{ $supervisor->Identifiant_email }}')"
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#resetModal"
                                                    title="Réactiver">
                                                    <svg class="bi" width="20" height="20" fill="currentColor" viewBox="0 0 20 20">
                                                        <path fill-rule="evenodd" d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z" clip-rule="evenodd" />
                                                    </svg>
                                                </button>
                                            @else
                                                <button type="button" class="btn btn-link text-danger p-0 border-0"
                                                    onclick="setDeleteAction('{{ route('supervisors.destroy', $supervisor->ID) }}', '{{ $supervisor->Identifiant_email }}')"
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#deleteModal"
                                                    title="Désactiver">
                                                    <svg class="bi" width="20" height="20" fill="currentColor" viewBox="0 0 20 20">
                                                        <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" />
                                                    </svg>
                                                </button>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center py-4">
                                        {{ __('Aucun responsable de service pour le moment.') }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>

                <!-- Modal Suppression -->
                <div class="modal fade" id="deleteModal" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">{{ __('Confirmation de désactivation') }}</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body">
                                <p>{{ __('Voulez-vous vraiment désactiver ce responsable de service ?') }}</p>
                                <p class="fw-bold" id="details_supervisor"></p>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __('Annuler') }}</button>
                                <form id="deleteForm" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger">{{ __('Désactiver') }}</button>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- Modal Reset -->
                <div class="modal fade" id="resetModal" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">{{ __('Confirmation de réactivation') }}</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body">
                                <p>{{ __('Voulez-vous vraiment réactiver ce responsable de service ?') }}</p>
                                <p class="fw-bold" id="supervisor_"></p>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __('Annuler') }}</button>
                                <form id="resetForm" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-success">{{ __('Réactiver') }}</button>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- Pagination -->
                <div class="mt-1">
                    {{ $supervisors->links("pagination.custom") }}
                </div>
            </div>
        </div>
    </div>

    @push("scripts")
    <script>
        function setDeleteAction(url, name) {
            document.getElementById('deleteForm').action = url;
            document.getElementById('details_supervisor').textContent = name;
        }

        function setResetAction(url, name) {
            document.getElementById('resetForm').action = url;
            document.getElementById('supervisor_').textContent = name;
        }
    </script>
    @endpush
</x-app-layout>