...
<div class="col-md-6">
    ...
    <select id="job_title_id" name="job_title_id" class="form-select">
        ...
        @foreach($jobTitles as $job)
            <option value="{{ $job->id }}" {{ old('job_title_id', $employe->job_title_id) == $job->id ? 'selected' : '' }}>
                {{ $job->name }}
            </option>
        @endforeach
    </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentJobTitleId = "{{ old('job_title_id', $employe->job_title_id) }}";

    function loadJobTitles(departmentId) {
let url = '{{ route("admin.get.job-titles.by.department") }}?department_id=' + (departmentId || '');        fetch(url)
    ...
    // Charger initialement si un service est sélectionné
