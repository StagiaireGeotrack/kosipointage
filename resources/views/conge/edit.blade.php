{{-- resources/views/conges/edit.blade.php --}}
<x-app-layout>
    <x-slot name="header">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('conges.update', $conge->id) }}">
                        ...
                        <div class="col-lg-6">
                            ...
                            <select id="employee_id" name="employee_id" class="form-select mt-1" required>
                                ...
                                @foreach($employes as $employe)
                                    <option value="{{ $employe->ID }}" {{ old('employee_id', $conge->employee_id) == $employe->ID ? 'selected' : '' }}>
                        ...
                        <div class="col-lg-6">
                            ...
                            <select id="type_conge" name="type_conge" class="form-select mt-1" required>
                                ...
                                @foreach($typesConge as $type)
                                    <option value="{{ $type }}" {{ old('type_conge', $conge->type_conge) == $type ? 'selected' : '' }}>
                                        {{ $type }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="commentaire" name="commentaire" class="form-control mt-1" rows="4" placeholder="{{ __('Commentaire optionnel') }}">{{ old('commentaire', $conge->commentaire) }}</textarea>
