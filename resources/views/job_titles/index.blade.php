    ...
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="fw-semibold fs-4 text-dark mb-0">
                <i class="bi bi-person-badge"></i> {{ __('Gestion des Postes') }}
            </h2>
    ...
    <div class="p-2">
        <div class="card shadow-sm">
            <div class="card-body">
                        ...
                        <div class="col-md-3">
                            ...
                            <select id="hierarchy_level_id" name="hierarchy_level_id" class="form-select mt-1">
                                ...
                                @foreach($levels as $level)
                                    <option value="{{ $level->id }}" {{ request('hierarchy_level_id') == $level->id ? 'selected' : '' }}>
                        ...
                        <tbody>
                            @forelse ($jobTitles as $jt)
                                <tr>
                                    ...
                                    <td class="align-middle">
                                        <span class="badge bg-secondary">{{ $jt->code ?? '-' }}</span>
                                    </td>
                                    <td class="align-middle">
                                        @if($jt->hierarchyLevel)
                                            <span class="badge bg-primary">{{ $jt->hierarchyLevel->code }}</span>
                ...
                @if(isset($jobTitles) && method_exists($jobTitles, 'links'))
                    <div class="mt-3">
                        {{ $jobTitles->links('pagination.custom') }}
                    </div>
                @endif
            </div>
        </div>
